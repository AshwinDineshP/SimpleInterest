...
function App() {
  ...
  const validate= (e)=> {
    ...
    ///^[0-9a-zA-Z@]*{MIN,MAX}$/
    // - match() - check whether the regular expression matches with the given string - returns array when the exp matches with the string else returns null
    ...
    <>
     <div style={{height: '100vh'}} className='bg-dark w-100 d-flex justify-content-center align-items-center'>
      <div style={{width:'500px'}} className='p-5 bg-light rounded'>
        ...
        <div className="mb-3">
          <TextField name='rate' id="outlined-basic" value={rate} label="Interest Rate (p.a)%" variant="outlined" className='w-100' onChange={(e)=>validate(e)} />
